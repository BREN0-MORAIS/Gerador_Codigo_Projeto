    ...
    public class Cryptography : ICryptography
    {
        ...
        private string DesCrypto(string texto)
        {
            try
            {
                ...
                return Encoding.ASCII.GetString(ConfigureCryptography(false).TransformFinalBlock(buff, 0, buff.Length));
            }
            catch
            {
                //MessageBox.Show(ex.Message, "GEOCID ", MessageBoxButtons.OK, MessageBoxIcon.Error);
        ...
        public string ConvertHexa(string asciiString)
        {
            ...
            {
                int tmp = num;
                numero += string.Format("{0:x2}", Convert.ToUInt32(tmp.ToString()));
            }
            return numero;
        }
        #endregion
